...
def detect_linear_region(
    *,
    ...
    plateau_result: DirectedPlateauResult,
) -> LinearRegionResult:
    ...
    raw_inverse_c2 = 1.0 / np.square(capacitance)

    detection_inverse_c2 = _smooth_inverse_c2(
        raw_inverse_c2
    )
    ...
    plateau = plateau_result.accumulation_plateau

    search_start = min(
        plateau.end_index + 1,
        len(voltage) - _MIN_ADAPTIVE_POINTS,
    )
    search_end = len(voltage) - 1

    if search_start >= search_end:
        raise ValueError(
            "Unable to locate depletion region after the accumulation plateau."
        )
