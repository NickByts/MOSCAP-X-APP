...
Phase 2 tab for displaying the calculated MOSCAP-X
semiconductor/device parameters.

No calculation logic is performed here.
This widget only displays a Phase2Results object.
"""
...
class Phase2Tab(QWidget):
    """Display Phase 2 extraction results."""
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._cards = [
            MetricCard(label)
            for label, _, _ in _METRIC_SPECS
        ]
    ...
    def set_summary(
        ...
        """
        Display a Phase2Results object.

        If summary is None, all metric cards are reset and
        the supplied error/status message is displayed.
        """
        ...
        if summary is None:
            self._status_label.setText(
                error_message
                or (
                    "Phase 2 requires valid Phase 1B "
                    "substrate and doping outputs."
                )
            )
